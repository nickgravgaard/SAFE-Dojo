...
let update msg model =
    ...
    | PostcodeChanged p ->
        let validationError =
            match (p = "", Validation.isValidPostcode p) with
            | (true, _) -> Some "Postcode must not be blank"
            | (false, true) -> None
            | (false, false) -> Some "Invalid postcode"
        { model with
            Postcode = p
